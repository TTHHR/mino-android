...
/// Outbound protocol used by local connections.
#[derive(Debug, Clone)]
pub enum OutboundProtocol {
    Shadowsocks,
    Mino(MinoOutboundConfig),
}
...
#[derive(Debug, Clone)]
pub struct MinoOutboundConfig {
    pub encoder: MinoEncoder,
    pub username: String,
    pub password: String,
    pub xor_mod: usize,
}
...
pub enum MinoEncoder {
    None,
    Xor,
}

fn parse_mino_protocol(protocol: Option<&str>, method: &str, encoder: Option<&str>, password: Option<&str>) -> Option<OutboundProtocol> {
    let method = method.to_ascii_lowercase();
    let protocol = protocol.unwrap_or_default().to_ascii_lowercase();
    if protocol != "mino" && method != "mino" && method != "mino+xor" {
        return None;
    }

    let encoder = match encoder.map(str::to_ascii_lowercase).as_deref() {
        Some("none") => MinoEncoder::None,
        Some("xor") | None if method == "mino+xor" || protocol == "mino" => MinoEncoder::Xor,
        None => MinoEncoder::None,
        Some(_) => return None,
    };
    let auth = password.unwrap_or_default();
    let (username, password) = auth.split_once(':').unwrap_or((auth, ""));
    Some(OutboundProtocol::Mino(MinoOutboundConfig {
        encoder,
    ...
    fn load_from_ssconfig(config: SSConfig, config_type: ConfigType) -> Result<Config, Error> {
        ...
        match config_type {
            ConfigType::Local => {
                ...
                if let Some(local_port) = config.local_port {
                    ...
                    local_config.protocol = match config.protocol.as_deref() {
        ...
        match (config.server, config.server_port, config.password, &config.method) {
            (Some(address), Some(port), pwd_opt, Some(m)) => {
                let outbound_protocol =
                    parse_mino_protocol(config.protocol.as_deref(), m, config.encoder.as_deref(), pwd_opt.as_deref());
                ...
                let method = if outbound_protocol.is_some() {
                    CipherKind::NONE
                } else {
                ...
                let password = if outbound_protocol.is_some() {
                    String::new().into()
                } else { match pwd_opt {
                ...
                if let Some(protocol) = outbound_protocol {
                    server_instance.protocol = protocol;
                }
        ...
        if let Some(servers) = config.servers {
            for svr in servers {
                ...
                let outbound_protocol = parse_mino_protocol(
                    svr.protocol.as_deref(),
                    &svr.method,
                    svr.encoder.as_deref(),
                    svr.password.as_deref(),
                );
                ...
                let method = if outbound_protocol.is_some() {
                    ...
                    match svr.method.parse::<CipherKind>() {
                        Ok(m) => m,
                        Err(..) => {
                            let err = Error::new(
                                ErrorKind::Invalid,
            ...
            _ => {
                ...
                for inst in &self.server {
                    ...
                    vsvr.push(SSServerExtConfig {
                        ...
                        protocol: match &inst.protocol {
                            OutboundProtocol::Shadowsocks => None,
                            OutboundProtocol::Mino(..) => Some("mino".to_owned()),
                        },
                        encoder: match &inst.protocol {
                            OutboundProtocol::Mino(mino) => Some(match mino.encoder {
...
mod mino_tests {
    ...
    #[test]
    fn load_standard_mino_xor_config() {
        ...
        .unwrap();
        match config.server[0].protocol {
            OutboundProtocol::Mino(ref mino) => {
                assert_eq!(mino.encoder, MinoEncoder::Xor);
                assert_eq!(mino.username, "user");
                assert_eq!(mino.password, "pass");
            }
            OutboundProtocol::Shadowsocks => panic!("mino config fell back to shadowsocks"),
        }
    }
}

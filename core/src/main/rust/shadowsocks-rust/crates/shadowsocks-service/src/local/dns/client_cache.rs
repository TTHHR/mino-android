...
impl DnsClientCache {
    ...
    pub async fn lookup_remote(
        ...
        self.lookup_dns(&key, msg, None, Some(context), Some(server)).await
    }
            ...
            let create_fn = async {
                match dck {
                    ...
                    DnsClientKey::TcpRemote(tcp_l) => {
                        ...
                        let server = server.expect("server config is required for remote DNS");

                        DnsClient::connect_tcp_remote(context, server, tcp_l).await
                    }

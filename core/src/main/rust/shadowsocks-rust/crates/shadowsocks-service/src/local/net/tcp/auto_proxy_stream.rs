...
use crate::{
    config::OutboundProtocol,
    local::{context::ServiceContext, loadbalancing::ServerIdent},
    net::MonProxyStream,
};

use super::{auto_proxy_io::AutoProxyIo, mino_stream::MinoClientStream};

fn reject_android_vpn_internal(addr: &Address) -> io::Result<()> {
    let ip = match addr {
        Address::SocketAddress(socket_addr) => socket_addr.ip(),
        Address::DomainNameAddress(..) => return Ok(()),
    };
    let is_internal = match ip {
        IpAddr::V4(ip) => {
            let octets = ip.octets();
            octets[0] == 172 && octets[1] == 19 && octets[2] == 0 && octets[3] <= 3
        }
        IpAddr::V6(ip) => {
            let base = Ipv6Addr::new(0xfdfe, 0xdcba, 0x9876, 0, 0, 0, 0, 0);
            let ip = u128::from(ip);
    ...
    if is_internal {
        return Err(io::Error::new(
            ...
            format!("android vpn internal address {addr} is not proxied"),
        ));
    }
    Ok(())
}
...
impl AutoProxyClientStream {
    ...
    {
        let addr = addr.into();
        reject_android_vpn_internal(&addr)?;
        ...
        A: Into<Address>,
    {
        ...
        match &server.server_instance_config().protocol {
            ...
            OutboundProtocol::Mino(mino_config) => {
                let stream = match MinoClientStream::connect(&context, server, addr, mino_config, connect_opts).await {
                    Ok(s) => s,
                    Err(err) => {
                        server.tcp_score().report_failure().await;
                        return Err(err);
                ...
                Ok(AutoProxyClientStream::Mino(stream))
            }
        }
    }
    ...
    pub fn local_addr(&self) -> io::Result<SocketAddr> {
        match *self {
            ...
            AutoProxyClientStream::Mino(ref s) => s.local_addr(),
    ...
    pub fn set_nodelay(&self, nodelay: bool) -> io::Result<()> {
        match *self {
            ...
            AutoProxyClientStream::Mino(ref s) => s.set_nodelay(nodelay),
...
impl AutoProxyIo for AutoProxyClientStream {
    fn is_proxied(&self) -> bool {
        matches!(*self, AutoProxyClientStream::Proxied(..) | AutoProxyClientStream::Mino(..))
    }
}
...
impl AsyncRead for AutoProxyClientStream {
    fn poll_read(self: Pin<&mut Self>, cx: &mut task::Context<'_>, buf: &mut ReadBuf<'_>) -> Poll<io::Result<()>> {
        match self.project() {
            ...
            AutoProxyClientStreamProj::Mino(s) => s.poll_read(cx, buf),
...
impl AsyncWrite for AutoProxyClientStream {
    fn poll_write(self: Pin<&mut Self>, cx: &mut task::Context<'_>, buf: &[u8]) -> Poll<io::Result<usize>> {
        match self.project() {
            ...
            AutoProxyClientStreamProj::Mino(s) => s.poll_write(cx, buf),
    ...
    fn poll_flush(self: Pin<&mut Self>, cx: &mut task::Context<'_>) -> Poll<io::Result<()>> {
        match self.project() {
            ...
            AutoProxyClientStreamProj::Mino(s) => s.poll_flush(cx),
    ...
    fn poll_shutdown(self: Pin<&mut Self>, cx: &mut task::Context<'_>) -> Poll<io::Result<()>> {
        match self.project() {
            ...
            AutoProxyClientStreamProj::Mino(s) => s.poll_shutdown(cx),
    ...
    fn poll_write_vectored(
        ...
        match self.project() {
            ...
            AutoProxyClientStreamProj::Mino(s) => s.poll_write_vectored(cx, bufs),

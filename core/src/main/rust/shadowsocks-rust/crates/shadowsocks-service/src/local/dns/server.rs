...
impl DnsBuilder {
    ...
    pub async fn build(self) -> io::Result<Dns> {
        ...
        let enable_udp_listener = self.mode.enable_udp() || cfg!(target_os = "android");
...
impl DnsClient {
    ...
    async fn acl_lookup(
        ...
        if local_addr_matches_remote(local_addr, remote_addr) {
            let local_response = self.lookup_local(query, local_addr).await;
            trace!("pick local response (direct dns): {:?}", local_response);
    ...
    async fn lookup_remote_inner(&self, query: &Query, remote_addr: &Address) -> io::Result<Message> {
        ...
        match self.mode {
            ...
            Mode::UdpOnly => {
                ...
                self.client_cache
                    .lookup_remote(&self.context, &server, remote_addr, message, true)
                    .await
                    .map_err(From::from)
            }
            Mode::TcpAndUdp => {
                ...
                let tcp_fut = async {
                    ...
                    let server = self.balancer.best_tcp_server();
                    self.client_cache
                        .lookup_remote(&self.context, &server, remote_addr, message2, false)
                        .await
                };
                let udp_fut = async {
                    let server = self.balancer.best_udp_server();
...
fn local_addr_matches_remote(local_addr: &NameServerAddr, remote_addr: &Address) -> bool {
    match (local_addr, remote_addr) {
        (NameServerAddr::SocketAddr(local), Address::SocketAddress(remote)) => local == remote,
        _ => false,
    }
}

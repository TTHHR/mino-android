...
data class Profile(
        ...
        var remoteDns: String = "180.76.76.76",
                ...
                if (uri.scheme.equals("mino", true)) {
                    try {
                        ...
                        if (host.isEmpty() || port <= 0) {
                            ...
                            profile.method = when (uri.getQueryParameter("encoder")?.lowercase(Locale.ENGLISH)) {
                                null, "", "xor" -> "mino+xor"
                                "none" -> "mino"
                                else -> {
                                    Timber.e("Unsupported mino encoder: ${it.value}")
                                    return@map null
                                }
                            }
                            profile.host = if (host.firstOrNull() == '[' && host.lastOrNull() == ']') {
                                host.substring(1, host.length - 1)
                            } else host
                            profile.remotePort = port
                            profile.password = javaURI.userInfo ?: ""
    ...
    fun toUri(): Uri {
        if (method.isMinoMethod()) {
            val wrappedHost = if (host.contains(':')) "[$host]" else host
            val builder = Uri.Builder()
                    .scheme("mino")
                    .encodedAuthority("${if (password.isEmpty()) "" else "$password@"}$wrappedHost:$remotePort")
            if (method == "mino+xor") builder.appendQueryParameter("encoder", "xor")
    ...
    fun toJson(profiles: LongSparseArray<Profile>? = null): JSONObject = JSONObject().apply {
        ...
        if (method.isMinoMethod()) {
            put("protocol", "mino")
            put("encoder", if (method == "mino+xor") "xor" else "none")
        }

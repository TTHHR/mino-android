...
public abstract class Tunnel {
	...
	final static ByteBuffer GL_BUFFER=ByteBuffer.allocate(Short.MAX_VALUE);
	...
	public void connect(InetSocketAddress destAddress) throws Exception{
		if(AmService.Instance.protect(m_InnerChannel.socket())){//保护socket不走vpn
	...
	public static final String bytesToHexString(byte[] bArray,int end) {
	...
	public void onConnectable(){
    	try {
			...
			MainActivity.mainActivity.addLog("连接错误:"+e);
			this.dispose();
		}
    }
	...
	public void onReadable(SelectionKey key){
		try {
			...
			MainActivity.mainActivity.addLog("读错误:"+e);
			e.printStackTrace();
			this.dispose();
		}
	}
	...
	public void onWritable(SelectionKey key){
		try {
			...
			MainActivity.mainActivity.addLog("写错误:"+e);
			this.dispose();
		}
	}

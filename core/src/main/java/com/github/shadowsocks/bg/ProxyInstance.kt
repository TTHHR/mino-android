...
class ProxyInstance(val profile: Profile, private val route: String = profile.route) {
    init {
        require(profile.host.isNotEmpty() &&
                (profile.method == "none" || profile.method.startsWith("mino") || profile.password.isNotEmpty())) {
    ...
    fun start(service: BaseService.Interface, stat: File, configFile: File, mode: String, dnsRelay: Boolean = true) {
        ...
        config.put("locals", JSONArray().apply {
            ...
            if (dnsRelay) try {
                ...
                val dnsHost = dns.host ?: "0.0.0.0"
                val dnsPort = if (dns.port < 0) 53 else dns.port
                val isMino = profile.method.startsWith("mino")
                put(JSONObject().apply {
                    put("local_address", DataStore.listenAddress)
                    put("local_port", DataStore.portLocalDns)
                    put("local_dns_address", if (isMino) dnsHost else "local_dns_path")
                    if (isMino) put("local_dns_port", dnsPort)
                    put("remote_dns_address", dnsHost)
                    put("remote_dns_port", dnsPort)
                    put("protocol", "dns")
                    if (isMino) put("mode", "tcp_only")
                })
            }
        })
